...
def process_files(input_folder, output_base_dir, num_best):
    # Dynamically append the `best<num_best>` level in the output directory structure
    input_folder_name = os.path.basename(os.path.normpath(input_folder))
    best_dir = os.path.join(output_base_dir, f"best{num_best}")  # Additional directory for num_best
    os.makedirs(best_dir, exist_ok=True)

    output_folder = os.path.join(best_dir, f"{input_folder_name}_best{num_best}")
    ...
    print(f"Files in '{input_folder}' to be processed for num_best={num_best}:")
...
if __name__ == "__main__":
    ...
    for num_best in range(1, args.num_best+1):  # Loop through num_best values from 1 to 20
        print(f"Processing with num_best={num_best}")
        process_files_in_directory(args.input_dir, args.output_dir, num_best)
